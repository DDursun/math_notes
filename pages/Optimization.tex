...
\textbf{Goal:} Find coefficients that minimize the sum of squared errors for data points $(x_i, y_i)$.

\textbf{Linear fit:} $\hat{y} = a + bx$. Minimize $L(a,b) = \sum_{i=1}^{n}(y_i - a - bx_i)^2$

\textbf{1.1} Take partial derivatives and set to zero:
\[\frac{\partial L}{\partial a} = -2\sum(y_i - a - bx_i) = 0\]
\[\frac{\partial L}{\partial b} = -2\sum x_i(y_i - a - bx_i) = 0\]

\textbf{1.2} Simplify to normal equations and solve:
\[\sum y_i = na + b\sum x_i\]
\[\sum x_i y_i = a\sum x_i + b\sum x_i^2\]

\textbf{1.3} Matrix style:$\mathbf{A}^T\mathbf{A}\vec{c} = \mathbf{A}^T\vec{y}$ where $\mathbf{A} = \begin{pmatrix} 1 & x_1 \\ \vdots & \vdots \\ 1 & x_n \end{pmatrix}$,$\vec{c} = \begin{pmatrix} a \\ b \end{pmatrix}$
...
\textbf{2.1} Set $\frac{\partial E}{\partial a_i} = 0$ for each $i$.

\textbf{2.2} Apply Leibniz rule (move derivative inside): $\int_{x_1}^{x_2} 2[f(x) - \hat{f}(x)](-\phi_i(x))\, dx = 0$

\textbf{2.3} Normal equations:
...
\textbf{4.1} Write Lagrangian: $\mathcal{L}(x,y,\lambda) = f(x,y) - \lambda(g(x,y) - c)$

\textbf{4.2} Set $\nabla \mathcal{L} = 0$: solve $\mathcal{L}_x = 0$, $\mathcal{L}_y = 0$, $\mathcal{L}_\lambda = 0$

\textbf{4.3} Classify critical points by comparing $f$ values or using bordered Hessian.
...
\textbf{5.1} Identify the objective function (minimize/maximize) and constraint.

\textbf{5.2} Use constraints to reduce variables (substitution) or use Lagrange multipliers.

\textbf{5.3} Differentiate, set to zero, solve for critical points.

\textbf{5.4} Verify min/max using second derivative test or boundary analysis.
...
\textit{Beam problem:} Rod over fence (height $h$, distance $w$). Use similar triangles: $y = \frac{h(w+x)}{x}$, minimize $L^2 = (w+x)^2 + y^2$. 
