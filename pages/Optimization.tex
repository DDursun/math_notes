...
\textbf{1.2} Simplify to normal equations:
\[\sum y_i = na + b\sum x_i\]
\[\sum x_i y_i = a\sum x_i + b\sum x_i^2\]

\textbf{1.3} Matrix form: $\mathbf{A}^T\mathbf{A}\vec{c} = \mathbf{A}^T\vec{y}$ where $\mathbf{A} = \begin{pmatrix} 1 & x_1 \\ \vdots & \vdots \\ 1 & x_n \end{pmatrix}$, $\vec{c} = \begin{pmatrix} a \\ b \end{pmatrix}$
