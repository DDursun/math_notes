...
%----------------------------------------------
\textbf{General 2nd-Order Linear PDE:} $Au_{xx} + Bu_{xy} + Cu_{yy} + Du_x + Eu_y + Fu = G$

\textbf{Classification} (based on discriminant $B^2 - 4AC$):
...
\textbf{Example: Wave Equation via Laplace Transform}

PDE: $u_{tt} = c^2 u_{xx}$, $u(x,0) = f(x)$, $u_t(x,0) = g(x)$

Apply $\mathcal{L}$: $s^2U - sf(x) - g(x) = c^2 U_{xx}$

$U_{xx} - \frac{s^2}{c^2}U = -\frac{sf(x) + g(x)}{c^2}$
...
Orthogonality: $\int_0^L \sin\frac{n\pi x}{L}\sin\frac{m\pi x}{L}\,dx = \frac{L}{2}\delta_{nm}$

$\int_0^L \cos\frac{n\pi x}{L}\cos\frac{m\pi x}{L}\,dx = \begin{cases} L & n=m=0 \\ L/2 & n=m\neq 0 \\ 0 & n\neq m\end{cases}$

$\int_0^L \sin\frac{n\pi x}{L}\cos\frac{m\pi x}{L}\,dx = 0$ (all $n,m$)
