\twocolumn[
\begin{center}
\textbf{\Large Series Solutions ODEs}
\end{center}
\vspace{4pt}]
...
Form: $y'' + p(x)y' + q(x)y = r(x)$ about $x_0$ where $p(x), q(x)$ are analytic.
...
Use when $p(x)$ or $q(x)$ have singularities ($\frac{1}{x}$, $\frac{1}{x^2}$, etc.).

Point $x_0$ is a \textbf{regular singular point} if $(x-x_0)p(x)$ and $(x-x_0)^2 q(x)$ are analytic at $x_0$.
